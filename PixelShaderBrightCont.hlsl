...
float4 main(VertexToPixel input) : SV_TARGET
{
	...
	//float3 finalColor =image.Sample(samplerOptions, input.uv).rgb + float3(0.1, 0.1, 0.1);//this value would have to be passed in through 
	float3 finalColor=((image.Sample(samplerOptions, input.uv).rgb - 0.5f)* max(2, 0)) + 0.5f;
